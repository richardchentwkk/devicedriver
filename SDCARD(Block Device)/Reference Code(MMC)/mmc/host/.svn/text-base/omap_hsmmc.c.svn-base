...
static void omap_hsmmc_do_irq(struct omap_hsmmc_host *host, int status, int irq)
{
	...
	if (status & ERR) {
		...
		if ((status & CMD_TIMEOUT) ||
			...
			if(status & CMD_CEB)
				printk("Catch CMD_CEB!\n");
			
			printk("CMD Timeout/CRC/CEB fail!");
			if (host->cmd) {
				if (status & CMD_TIMEOUT) {
		...
		if ((status & DATA_TIMEOUT) ||
			...
			if(status & DATA_DEB)
				printk("Catch DATA_ENDBIT!!\n");
			printk("DATA Timeout/CRC/DATA_ENDBIT fail!\n");
			if (host->data || host->response_busy) {
		...
		if (status & CARD_ERR) {
			printk(	"Ignoring card err CMD%d\n", host->cmd->opcode);
			if (host->cmd)
				end_cmd = 1;
			if (host->data)
				end_trans = 1;
...
static void omap_hsmmc_irq_tasklet(unsigned long arg)
{
    ...
    omap_hsmmc_do_irq(host, host->irq_stat,host->irq);
...
static irqreturn_t omap_hsmmc_irq(int irq, void *dev_id)
{
	...
	host->irq_stat = OMAP_HSMMC_READ(host->base, STAT);
	tasklet_schedule(&host->irq_tasklet);
...
static void omap_hsmmc_detect(struct work_struct *work)
{
	...
	if (host->suspended){
		printk("omap_hsmmc_detect, suspend return \n");
		return;
	}
	...
	printk("WQ: omap_hsmmc_detect..carddetect%d\n",carddetect);
	if (carddetect)
...
static irqreturn_t omap_hsmmc_cd_handler(int irq, void *dev_id)
{
	...
	printk("IRQ:map_hsmmc_cd_handler...%d\n",irq);
	if (host->suspended)
		return IRQ_HANDLED;
...
static int __init omap_hsmmc_probe(struct platform_device *pdev)
{
	...
	tasklet_init(&host->irq_tasklet,omap_hsmmc_irq_tasklet,(unsigned long)host);
